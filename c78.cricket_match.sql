--day78: how many matches were played by each player and
--in how many of these batted and in how many of these they bowled.
...
--Method 1:
with all_players as (
select player, count(matchid) as total_matches_played
from
(
...
left join 
(
select batter, count(distinct matchid) as batting_matches
from cricket_match
group by batter
) bat on ap.player=bat.batter
left join
(select bowler, count(distinct matchid) as bowling_matches
from cricket_match
group by bowler
) bowl on ap.player=bowl.bowler;



--Method 2:
with cte as (
select batter as player, matchid as batting_match_id, null as bowling_match_id
from cricket_match
union 
select bowler as player, null as batting_match_id, matchid as bowling_match_id
from cricket_match
)
select player
, count (distinct coalesce(batting_match_id, bowling_match_id)) as total_matches_played
, count(batting_match_id) as batting_matches
, count(bowling_match_id) as bowling_matches
from cte
group by player;
