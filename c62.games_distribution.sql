--Day62: get a distribution of games (% of total games) based on the social interaction happening during the games.
...
case 
 ...
 when count(distinct case when interaction_type is not null then user_id end)=2 AND 
      count(distinct case when interaction_type='custom_typed' then user_id end)=0 then 'Both sided interaction without custom_typed_messages'
 when count(distinct case when interaction_type is not null then user_id end)=2 AND 
      count(distinct case when interaction_type='custom_typed' then user_id end)>=1 then 'Both sided interaction with custome_typed messages from at least one player'
end as game_type
